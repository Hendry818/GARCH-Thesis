...
# probabilities for the simulated paths to reach the maximum value of Hang Seng Index.

## OPTIONS

set.seed(20201231) # fix seed for simulations (20201231 replicates the paper's results)
...
tab_prob <- df_sim %>%
  group_by(ref_date) %>%
  summarise(prob = mean(sim_price > max(df_prices$price.close)))
...
p1 <- ggplot() + 
  geom_line(data = df_prices_temp, 
            aes(x = ref.date, y = price.close), color = 'black', size = 0.75)  + 
  ...
  geom_hline(yintercept = max(df_prices_temp$price.close)) + 
  labs(title = paste0('Price Projections of ', series_name),
       ...
       x = '',
       y = 'Value') + 
  ylim(c(0.75*min(df_prices_temp$price.close), 
         1.25*max(df_prices_temp$price.close))) + 
...
df_textbox <- tibble(ref_date = df_date$ref_date[2],
                     ...
                     label = paste0('According to the estimated _', my_garch_name, '_ model, ', 
                                    ...
                                    format(max(df_prices$price.close), 
                                           big.mark = ',',
                                           decimal.mark = '.'),
